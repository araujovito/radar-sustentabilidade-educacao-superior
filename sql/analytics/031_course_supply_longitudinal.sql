...
-- Mart de oferta longitudinal (2014-2024).
--
-- Mesma reconciliação validada em 2024, aplicada ano a ano: o presencial vem
-- da dimensão 1; na EAD, capacidade e demanda vêm da dimensão 3 e alunos da
-- dimensão 2. A dimensão 4 (oferta no exterior) fica fora do recorte
-- brasileiro em todas as edições.
--
-- Todas as agregações filtram dimensão explicitamente, conforme a regra
-- registrada em docs/layout_changes.md: a convenção para medidas inaplicáveis
-- mudou em 2020 e não se pode confiar na ausência de nulos.

CREATE OR REPLACE VIEW analytics.course_supply AS
WITH presencial AS (
    SELECT
        census_year,
        institution_id,
        ...
        teaching_modality,
        MAX(education_network) AS education_network,
        MAX(administrative_category) AS administrative_category,
...
-- Indicadores transversais por oferta e ano. As mesmas definições de 2024,
-- agora disponíveis para toda a série.
CREATE OR REPLACE VIEW analytics.course_sustainability AS
SELECT
    supply.*,
    ...
    graduates::NUMERIC / NULLIF(enrollments, 0) AS graduation_intensity,
...
FROM analytics.course_supply AS supply
LEFT JOIN staging.institutions AS institution
